...
NOTE: El backend `ontap-san-economy` importa un LUN a la vez. Puedes automatizar varias importaciones usando un script.
...
--
Requisito de nomenclatura LUN:: El LUN debe llamarse `lun0`.
Ejemplo:: Si tu `storagePrefix` es `xyz`, la ruta completa al LUN es:
+
--
[listing]
----
