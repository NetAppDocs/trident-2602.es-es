...
NOTE: Este controlador está dedicado a cargas de trabajo en bloque y no es compatible con protocolos NAS.
...
NOTE: El controlador SAN solo es compatible con el nivel de servicio Flex y no utiliza parámetros de backend específicos de NAS como  `exportRule`,  `unixPermissions`,  `nasType`,  `snapshotDir`,  `nfsMountOptions` o configuraciones relacionadas con el tiering.
